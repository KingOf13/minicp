    ...
    public static List<Integer> solve(int n, int[][] w, int[][] d, boolean verbose, Predicate<SearchStatistics> limit) {
        ...
        */
        ...
        DFSearch dfs = makeDfs(cp, () -> {
              IntVar sel = selectMin(x,// pas eu le time=> Remplacer x par un tableau des pair (i,j) et utiliser selectMin pour trouver le noeud avec le poinds le plus éloigné et la location correcte
                 vary -> vary.size() > 1, // filter
                 vary -> vary.min()     // variable selector
              );
